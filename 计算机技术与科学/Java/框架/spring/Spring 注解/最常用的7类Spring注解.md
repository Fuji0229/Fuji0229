...
## 7.@Value
注解使用在字段，构造器参数和方法参数上。@Value可以指定属性取值的表达式，支持通过#{}使用SpringEL取值，也支持使用${}将属性来源中properties文件、本地环境变量、系统属性的值注入到bean的属性中，此注解值的注入发生在AutowireAnnotationBeanPostProcessor类中。
...
此外，Spring4.3之后引入了一系列@RequestMapping的变种。如下：@GetMapping
@PostMapping
@PutMapping
@PatchMapping
@DeleteMapping
...
// controller和上面的代码在同一controller中@Controller
