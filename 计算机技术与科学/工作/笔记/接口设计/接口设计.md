...
```
24.接口实现过程汇总，注意大文件，大事件，大对象
+ 读取大文件时，不要使用Files.readAllBytes直接读取到内存，会造成OOM,建议使用BufferReader一行一行的读取。
+ 大事务可能会造成死锁、回滚时间长、主从延迟等、开发中尽量避免大事务。
+ 注意大对象的使用，因为大对象是直接进入老年代，会出发fullgc

25.接口考虑限流
突然增高的并发请求
限流定义：
	控制网络接口发送或者接受请求的速率，防止Dos攻击和限制web爬虫。限流也称流量控制，指系统在高并发或者大流量的请求下，限制新的请求对系统的访问，从而保证系统的稳定性。
限流方案：
Guava的RateLimter单机版限流
Redis分布式限流
sentinel限流

26.代码运行时，注意运行时异常（NPE，下标越界等）
日常开发中规避数组下标越界，被零整除，空指针等运行时错误
```java
...
27.保证接口安全性
如果API是对外提供的，需要保证接口的安全性。
方案：
	token机制
	![[Pasted image 20220913144131.png]]
	接口签名
	接口请求的相关信息（请求报文、请求时间戳、版本号、AppID等），客户端私钥加密，然后服务端用公钥验证，验证通过才认为是合法的，没有被篡改过的请求。
	加签验签
	服务层对报文加密：
		参考Https原理，服务端把公钥给客户端，然后客户端生成对称秘钥。客户端用对称秘钥进行加密请求报文，服务端用对应的秘钥解密报文。
接口的安全性还包括：
	手机、身份证的脱敏。（用户的隐私数据不能随便暴露）

28.分布式事务
分布式事务：
	指事务的参与者，支持事务的服务器、资源服务器以及事务的管理器分别位于分布式系统的不同节点之上，为了保证不同数据库节点的数据一致性。
分布式事务的解决方案：
	- 2PC（二阶段提交）、3PC
...
- 方法的访问权限必须是public
- 方法比fianl修饰
- 在同一个类中的方法直接内部调用，会导致事务失败
- 方法没有交给spring管理，不会生成spring事务
- 多线程调用，两个方法不在一个线程中
- 表的存储引擎不支持事务
- try catch误吞异常
- 错误的传播特性
...
30.掌握常用的设计模式
设计模式是代码设计经验的总结，使用设计模式可以重用代码，让代码更容易让人理解，保证代码的可靠性。

31.线程安全问题
案例:高并发环境下，hashmap可能会出死循环，因为它是非线程安全的。可以考虑ConcurrentHashmap。
...
33.接口的版本控制
请求基础报文，应该包含version接口版本号字段，方便未来做接口兼容
...
36.沟通（前端、产品）
客户端对齐接口，难点跟技术leader对齐方案，实现需求的过程中，及时和产品沟通